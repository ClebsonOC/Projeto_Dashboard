...
export async function initFinanceiroPage(applyFiltersCallback, populateFilterDropdownsCallback) {
    ...
    try {
        ...
        dataRows.forEach(row => {
            ...
            // Verifica se a coluna "MEDIÇÃO" está vazia ou contém apenas espaços
            if (!medicaoOriginal || medicaoOriginal.trim() === '') {
                return; // Pula esta linha, não a adiciona a processedRowsData
            }
            const medicao = medicaoOriginal; // Se chegou aqui, medicaoOriginal é válido
            // ✅ **FIM DA MODIFICAÇÃO**
            ...
            // const medicao = row[headerMap.get(CSV_HEADERS.MEDICAO)] || ''; // Linha original removida/substituída pela lógica acima
            
            const periodoCompleto = row[headerMap.get(CSV_HEADERS.PERIODO)] || '';
            let periodoFormatado = '';
            if (periodoCompleto) {
                let dateObject;
                const partsDate = periodoCompleto.split('/');
                if (partsDate.length === 3) {
                    ...
                    if (!isNaN(dateObject) && dateObject.getDate() === day && dateObject.getMonth() === month && dateObject.getFullYear() === year) {
                        periodoFormatado = new Intl.DateTimeFormat('pt-BR').format(dateObject);
                    }
                } else if (partsDate.length === 2) {
                    const month = parseInt(partsDate[0], 10) - 1;
                    const year = parseInt(partsDate[1], 10);
                    dateObject = new Date(year, month, 1);
                    if (!isNaN(dateObject) && dateObject.getMonth() === month && dateObject.getFullYear() === year) {
                        periodoFormatado = `${(month + 1).toString().padStart(2, '0')}/${year}`;
                    }
                }
            }
            const periodo = periodoFormatado || periodoCompleto;
